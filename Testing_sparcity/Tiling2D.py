...
def Tiling2D(tensor1, tensor2,number_of_cluster, number_of_MS_per_cluster, number_of_CTA_per_MS, scheduling_protocol,tiling = 0,faulty_MS = -1, block_executed_by_fauly_MS = None, Scheduler_result = []):
    x1,y1 = tensor1.size()
    x2,y2 = tensor2.size()
        ...
        for ms in range(n_ms):
            for ns in range(n_ns):
                for ks in range(n_ks):
                    ...
                    CTA_list.append((A,B)) 
        #Scheduler
        result_list= Scheduler(CTA_list,number_of_cluster, number_of_MS_per_cluster,number_of_CTA_per_MS,scheduling_protocol,faulty_MS, block_executed_by_fauly_MS, Scheduler_result)
